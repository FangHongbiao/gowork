...
func NewUserManagerHandler() *UserManager {
	return &UserManager{}
}

func (p *UserManager) Ping(ctx context.Context) (err error) {
	fmt.Print("ping()\n")
	return nil
}

func (p *UserManager) GetUserInfo(ctx context.Context, userId user.Int) (r *user.UserInfo, err error) {
	fmt.Print("getUserInfo()\n")
	...
	error := user.NewUserNotExist()
	error.Why = "用户idbucunz"
	return nil, error
}