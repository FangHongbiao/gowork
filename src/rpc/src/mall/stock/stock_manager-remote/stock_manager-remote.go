...
func Usage() {
  ...
  fmt.Fprintln(os.Stderr, "  bool changeStock(StockRequest request)")
  fmt.Fprintln(os.Stderr)
  os.Exit(0)
}
...
func main() {
  ...
  client := stock.NewStockManagerClient(thrift.NewTStandardClient(iprot, oprot))
  ...
  case "changeStock":
    if flag.NArg() - 1 != 1 {
      fmt.Fprintln(os.Stderr, "ChangeStock requires 1 args")
      flag.Usage()
    }
    arg6 := flag.Arg(1)
    ...
    argvalue0 := stock.NewStockRequest()
    ...
    value0 := argvalue0
    fmt.Print(client.ChangeStock(context.Background(), value0))
    fmt.Print("\n")
    break
  case "":
    Usage()
